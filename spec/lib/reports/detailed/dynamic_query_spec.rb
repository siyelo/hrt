  ...
  describe "budget report" do
    ...
    context "complex reports" do
      ...
      it "should adjust the total amounts as per codings (2 coding budget splits)" do
        ...
        @purpose_budget_split1 = FactoryGirl.create :purpose_budget_split,
          :percentage => 20, :activity => @activity, :code => @purpose_code1
        @location_budget_split = FactoryGirl.create :location_budget_split,
          :percentage => 100, :activity => @activity, :code => @location_code1
        table = run_report
        table[0]['Funding Source'].should == @organization.name
      ...
      it "should adjust the total amounts as per codings (2 location budget splits)" do
        ...
        @budget_purpose = FactoryGirl.create :budget_purpose, :percentage => 100,
          :activity => @activity, :code => @purpose_code1
        @location_budget_split = FactoryGirl.create :location_budget_split,
          :percentage => 70, :activity => @activity, :code => @location_code1
        @location_budget_split = FactoryGirl.create :location_budget_split,
          :percentage => 30, :activity => @activity, :code => @location_code1
        table = run_report
        table[0]['Funding Source'].should == @organization.name
        table[0]['Data Source'].should == @organization.name
        table[0]['Implementer'].should == @is.organization.name
      ...
      it "should adjust the total amounts as per codings (2 of each budget splits)" do
        @cost_categorization = FactoryGirl.create :input_budget_split,
          :percentage => 90, :activity => @activity, :code => @input_code1
        @cost_categorization1 = FactoryGirl.create :input_budget_split,
          :percentage => 10, :activity => @activity, :code => @input_code1
        @budget_purpose = FactoryGirl.create :budget_purpose, :percentage => 80,
          :activity => @activity, :code => @purpose_code1
        ...
        table[2]['Name of District'].should == @activity.locations.map(&:name).join(",")
        ...
        table[3]['Name of District'].should == @activity.locations.map(&:name).join(",")
        ...
        table[4]['Name of District'].should == @activity.locations.map(&:name).join(",")
        ...
        table[5]['Name of District'].should == @activity.locations.map(&:name).join(",")
        ...
        table[6]['Name of District'].should == @activity.locations.map(&:name).join(",")
        ...
        table[7]['Name of District'].should == @activity.locations.map(&:name).join(",")
      ...
      it "should adjust the total amounts as per codings (2 of each budget splits and 2 funders)" do
        ...
        table[8]['Name of District'].should == @activity.locations.map(&:name).join(",")
        ...
        table[9]['Name of District'].should == @activity.locations.map(&:name).join(",")
        ...
        table[10]['Name of District'].should == @activity.locations.map(&:name).join(",")
        ...
        table[11]['Name of District'].should == @activity.locations.map(&:name).join(",")
        ...
        table[12]['Name of District'].should == @activity.locations.map(&:name).join(",")
        ...
        table[13]['Name of District'].should == @activity.locations.map(&:name).join(",")
        ...
        table[14]['Name of District'].should == @activity.locations.map(&:name).join(",")
        ...
        table[15]['Name of District'].should == @activity.locations.map(&:name).join(",")
    ...
    context "currency conversion" do
      ...
      it "should correctly create the currency for a fake project for an activity/othercost" do
        ...
        @is = FactoryGirl.create :implementer_split, :activity => @other_cost,
          :organization => @organization, :budget => 100
        @cost_categorization = FactoryGirl.create :input_budget_split,
          :percentage => 100, :activity => @other_cost, :code => @input_code1
        @budget_purpose = FactoryGirl.create :budget_purpose, :percentage => 100,
          :activity => @other_cost, :code => @purpose_code1
        @location_budget_split = FactoryGirl.create :location_budget_split,
          :percentage => 100, :activity => @other_cost, :code => @location_code1
        ...
        table[0]['Location Split Total %'].should == 100.0
        table[0]['Location Split %'].should == 100.0
        table[0]['Name of District'].should == @other_cost.locations.map(&:name).join(",")
        table[0]['Total Amount ($)'].round(2).should == 50.00
        table[0]['Actual Double Count'].should == @is.double_count
    ...
    context "Incomplete classifications" do
      ...
      it "should create a inputs row with the outstanding amount (should make 2 rows)" do
        @cost_categorization = FactoryGirl.create :input_budget_split,
          :percentage => 80, :activity => @activity, :code => @input_code1
        @budget_purpose = FactoryGirl.create :budget_purpose, :percentage => 100,
          :activity => @activity, :code => @purpose_code1
        @location_budget_split = FactoryGirl.create :location_budget_split,
          :percentage => 100, :activity => @activity, :code => @location_code1
      ...
      it "should create a districts row with the outstanding amount (should make 2 rows)" do
        ...
        @budget_purpose = FactoryGirl.create :budget_purpose, :percentage => 100,
          :activity => @activity, :code => @purpose_code1
        @location_budget_split = FactoryGirl.create :location_budget_split,
          :percentage => 85, :activity => @activity, :code => @location_code1
      ...
      context "within allowed leeway" do
        it "should not create a inputs row with the outstanding amount (should make 1 row)" do
          @cost_categorization = FactoryGirl.create :input_budget_split,
            :percentage => 99.5, :activity => @activity, :code => @input_code1
          @budget_purpose = FactoryGirl.create :budget_purpose, :percentage => 100,
            :activity => @activity, :code => @purpose_code1
          @location_budget_split = FactoryGirl.create :location_budget_split,
            :percentage => 100, :activity => @activity, :code => @location_code1
