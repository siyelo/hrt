...
describe Reports::Detailed::Targets do
  def run_report(request, amount_type)
    content = Reports::Detailed::Targets.new(request, amount_type, 'xls').data
  ...
  [:budget, :spend].each do |amount_type|
    context "#{amount_type}" do
      before :each do
        ...
        FactoryGirl.create(:target, activity: @activity1, description: 'target1')
        FactoryGirl.create(:target, activity: @activity1, description: 'target2')
