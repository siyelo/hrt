...
class Admin::OrganizationsController < Admin::BaseController
  ...
  def remove_duplicate
    merger = OrganizationMerger.new(
      params[:target_organization_id], params[:duplicate_organization_id])
    if merger.merge
        render_notice("Organizations successfully merged.",
                        duplicate_admin_organizations_path)
    else
      render_error(merger.error, duplicate_admin_organizations_path)
    end
  end
